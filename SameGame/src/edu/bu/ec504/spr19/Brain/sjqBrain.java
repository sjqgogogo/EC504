...
public class sjqBrain extends Brain {
    // fields
    ...
    private pos chooseMove() {
        // greedy choice
        int maxCount=0;                  // the maximum number of points for the best position found
        int firstLevel = 7;
        int level = 0;
        ...
        for (int xx=0; xx<currState.columns(); xx++)
            for (int yy=0; yy<currState.rows(xx); yy++) {
                CircleColor currColor = currStateCopy.get(xx,yy);
                if(currColor == CircleColor.NONE)
                    continue;
                ...
                if ( currColor == CircleColor.Red) {
                    if (count == 1)
                        level = 3;
                    else
                        level = 1;
                    }
                else if (currColor == CircleColor.Green) {
                    if (count == 1)
                        level = 4;
                    else
                        level = 2;
                }
                else if (currColor == CircleColor.Blue) {
                    if (count == 1)
                        level = 4;
                    else
                        level = 2;
